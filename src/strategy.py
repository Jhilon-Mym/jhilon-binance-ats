...
def predict_signal(df):
    ...
    """
    # Prefer hybrid model if available
    hybrid_manifest = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'models', 'hybrid', 'manifest.json'))
    model_obj = None
...
def apply_strategy(history, ai_signal=None, threshold=0.8):
    ...
    # Market-high override: if price is near recent high and AI says SELL, force SELL
    try:
        recent_high = max([row.get('high', 0) for row in history[-50:]])
        last_close = history[-1].get('close', 0)
        if last_close > recent_high * 0.99 and side == 'SELL':
            final_side = 'SELL'
            logging.info(f"[STRATEGY] Market-high override: Forcing SELL (close={last_close}, recent_high={recent_high})")
    except Exception as e:
        logging.warning(f"[STRATEGY] Market-high override error: {e}")
    if final_side == "SELL":
