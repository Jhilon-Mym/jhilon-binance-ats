...
def get_pending_file():
    ...
    # return absolute path located at repo root
    candidate = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', fname))
    # Fallback to legacy `pending_trades.json` if the netmode-specific file isn't present
    if os.path.exists(candidate):
        return candidate
    legacy = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'pending_trades.json'))
    if os.path.exists(legacy):
        return legacy
    # return candidate path even if it doesn't exist yet (caller will create it)
    return candidate
