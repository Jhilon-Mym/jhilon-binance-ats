...
- Saves evaluation report to training/eval_report.json
"""
...
spec = importlib.util.spec_from_file_location('predict_hybrid', os.path.abspath(os.path.join(os.path.dirname(__file__), '../src/predict_hybrid.py')))
predict_hybrid = importlib.util.module_from_spec(spec)
spec.loader.exec_module(predict_hybrid)
HybridPredictor = predict_hybrid.HybridPredictor

import os
DATA_PATH = os.path.join(r"D:\binance_ats_clone\obaidur-binance-ats-main", 'klines_BTCUSDT_5m_balanced.csv')
REPORT_PATH = "training/eval_report.json"
